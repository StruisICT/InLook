...
| `fuzz/` | cargo-fuzz target `render_eml` over `render_file_to_html` (both formats; detached workspace, nightly-only). CI smoke-fuzzes it via `.github/workflows/fuzz.yml` (render-path PRs, weekly, manual). |
