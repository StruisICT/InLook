...
| `src/update.rs` | Windows-only, binary-only. **Opt-in** update check: consent stored in HKCU, WinHTTP (OS TLS) reads GitHub's latest-release redirect `Location` header, notifies once per new version with winget/download instructions. No network without consent; never downloads/runs anything. |
...
**Data flow:** `file path → read_eml() (≤50 MiB) → render_file_to_html() → served from memory via a custom protocol (`inlookview://`) → wry WebView`. The page is **not** loaded with `with_html` — that goes through WebView2's `NavigateToString`, which caps the page at 2 MB, so large/image-heavy emails failed. The custom protocol has no size limit and keeps the HTML off disk.
